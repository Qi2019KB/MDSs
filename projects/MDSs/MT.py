...
def train(trainLoader, mds1_model, mds1_model_ema, mds1_optim, args):
    ...
    for bat, (imgMapArray, kpsHeatmapArrays, meta) in enumerate(trainLoader):
        ...
        mds1_icc_loss += (mds1_icc_sum / mds1_icc_count) if mds1_icc_count > 0 else mds1_icc_sum
        ...
        # cal total loss
        mds1_total_loss = args.consWeight * (mds1_icc_loss + mds1_ecc_loss) + args.poseWeight * mds1_pec_loss
        # backward
        mds1_total_loss.backward()  # retain_graph=True
        mds1_optim.step()
        update_ema_variables(mds1_model, mds1_model_ema, args)  # update teacher by EMA

        # region 8. clearing the GPU Cache
        del kpsHeatmapArrays, kpsGateArrays, imgMapArray, warpmatArray, isflipArray, sampleWeightArray
        del outs_mds1, outs_mds1_ema
        del mds1_icc_loss, mds2_icc_loss, mds1_pec_loss
        del mds1_total_loss
        # endregion
...
def calSampleWeight_train(islabeledArray, args):
    ...
    sampleWeightArray_pseudo = [0. * torch.ones_like(sampleWeight) for sampleWeight in sampleWeightArray]  # 2 * [bs]
