...
class AugmentUtils:
    ...
    def affine_mulKps(cls, imgMap, kpsMapArray, center, scale, sf, angle, rf, matrixRes):
        ...
        return imgMap, kpsMapArray_new, scale, angle

    @classmethod
    def affine_back(cls, kpsHeatmap, warpmat, isflip):
        kpsHeatmap_back = kpsHeatmap.clone()
        # 进行反向仿射变换
        affine_grid = F.affine_grid(warpmat, kpsHeatmap_back.size(), align_corners=True)
        kpsHeatmap_back = F.grid_sample(kpsHeatmap_back, affine_grid, align_corners=True)
        # 进行反向水平翻转
        kpsHeatmap_back = torch.where(isflip[:, None, None, None], kpsHeatmap_back.detach().cpu().data, cls.fliplr_back(kpsHeatmap_back.detach().cpu().data))
        # # 从heatmap中获得关键点
        # bs, k, _, _ = kpsHeatmap_back.shape
        # kpsMap_warpmat = torch.zeros((bs, k, 3))
        # kpsMap_warpmat[:, :, 0:2] = proc.kps_fromHeatmap(kpsMap, cenMap, scale, matrixRes)
        # kpsMap_warpmat[:, :, 2] = kpsWeight
        return kpsHeatmap_back
    ...
    def fliplr(cls, imgMap, kpsMap, center, prob=0.5):
        ...
        if random.random() <= prob:
            ...
            isflip = True
        return imgMap, kpsMap, center, isflip
    ...
    def fliplr_mulKps(cls, imgMap, kpsMapArray, center, prob=0.5):
        ...
        return imgMap, kpsMapArray_new, center, isflip
